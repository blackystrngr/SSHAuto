...
class Monitor:
    ...
    def live_stats(self, sample_seconds=1.0):
        return LiveStats(
    ...
    def _active_connections(self):
        ...
        if not result.ok:
            return 0
        return len([l for l in result.stdout.splitlines()[1:] if l.strip()])

    def _throughput(self, sample_seconds):
    ...
    def speed_test(self):
        result = Shell.run(f'curl -o /dev/null -s -w "%{{speed_download}}" --max-time 10 "{SPEEDTEST_URL}"',
