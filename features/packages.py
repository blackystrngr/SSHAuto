...
class PackagesFeature(BaseFeature):
    ...
    def install(self) -> None:
        ...
        log.info(f"installing {len(REQUIRED_PACKAGES)} packages: {', '.join(REQUIRED_PACKAGES)}")
        try:
            Shell.run("DEBIAN_FRONTEND=noninteractive apt-get install -y " + " ".join(REQUIRED_PACKAGES),
                      timeout=600, retries=1)
        except Exception as exc:
            raise PackageError(f"apt install failed: {exc}")
        self._remove_conflicting()
        self._install_pip_packages()

    def remove(self) -> None:
        log.warning("packages feature does not uninstall required packages (too destructive)")

    def _core_check_list(self):
        return ["nginx", "dropbear", "openssh-server", "fail2ban",
                "certbot", "python3", "iptables", "git", "squid"]

    def _dpkg_installed(self, pkg):
    ...
    def _remove_conflicting(self):
        ...
        if not present:
            log.info("no conflicting packages")
            return
        ...
        Shell.run("DEBIAN_FRONTEND=noninteractive apt-get purge -y " + " ".join(present),
    ...
    def _install_pip_packages(self):
        if PIP_PACKAGES:
            ...
            Shell.run("pip3 install --break-system-packages -q " + " ".join(PIP_PACKAGES),
                      check=False, timeout=180)
