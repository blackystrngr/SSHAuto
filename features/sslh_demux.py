...
class SslhDemuxFeature(BaseFeature):
    ...
    def install(self) -> None:
        ...
        # Check if sslh is already installed
        if not Shell.run("which sslh", check=False).ok:
            log.info("sslh not found, installing...")
            for attempt in range(3):
                result = Shell.run("apt-get install -y sslh", check=False, timeout=30)
                if result.ok:
                    break
                if "Could not get lock" in result.stderr:
                    ...
                    raise Exception(f"Failed to install sslh: {result.stderr}")
            else:
                raise Exception("Failed to install sslh after retries.")
        else:
            log.info("sslh already installed.")
