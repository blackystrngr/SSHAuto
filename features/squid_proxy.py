...
class SquidProxyFeature(BaseFeature):
    ...
    depends_on = ["packages", "firewall"]  # no conflict with nginx
...
# You can add more ports below if they don't conflict with Nginx
# http_port 0.0.0.0:8081
# http_port 0.0.0.0:8881
...
"""
        ...
        for attempt in range(3):
            result = Shell.run("systemctl restart squid", check=False, timeout=30)
            if result.ok:
                break
            time.sleep(2)
        else:
            log.warning("Squid service did not start cleanly. Check 'journalctl -u squid'.")
        ...
        log.important("HTTP Injector can use Proxy Type: HTTP, IP: your_server_ip, Port: 3128.")
